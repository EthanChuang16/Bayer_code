...
segDirList = ['ADC2_Info_Seg_Combined','ADC2_Info_Seg_Combined']#'DWI_Extract2_Info_Seg_Combined']
...
df = df[df['Exclude'] == 0]
df = df[df['PatientID'] == 2031639]


#patientList = [2391715, 2570417]
#df = df[df['PatientID'].isin(patientList)]
##df = df[df['PatientID'] == 6500366]
...
for cti, i in enumerate(imgDir):
  ...
  for index, row in df.iterrows():
    ...
    if  (os.path.isfile(os.path.join(cropDirFig, mrn_padded + '.png'))):
      ...
      DILFunctions.plotimgOnly(cropimgsitk, figName, 3)
